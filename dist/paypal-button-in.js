...
function loadExhangeRate(callback) {
	...
	if (PAYPAL.exchangeRates) {
		return callback();
	}
	if (window.storeExchangeRate) {
		backupCallback = window.storeExchangeRate;
...
loadExhangeRate(function () {
	...
	if (!PAYPAL.apps.ButtonFactory) {
		...
		app.create = function (business, raw, type, parent) {
			...
			if (data.items.currency_code && data.items.currency_code.value === 'INR') {
				data.add('currency_code', 'USD');
				if (data.items.amount && data.items.amount.value) {
					data.add('amount', data.items.amount.value * PAYPAL.exchangeRates.USD);
				}
				if (data.items.shipping && data.items.shipping.value) {
					data.add('shipping', data.items.shipping.value * PAYPAL.exchangeRates.USD);
				}
				if (data.items.tax && data.items.tax.value) {
					data.add('tax', data.items.tax.value * PAYPAL.exchangeRates.USD);
				}
			}
